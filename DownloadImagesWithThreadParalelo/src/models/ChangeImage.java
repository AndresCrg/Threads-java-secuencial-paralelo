...
public class ChangeImage {
	...
	public void change(String image) {
		...
		try {
			// leemos la img
			originalImage = ImageIO.read(new File("dataBase/"+ image.substring(49, image.length())));
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		try {
			ImageIO.write(originalImage, "jpg",new File("dataBaseB&M/"+ image.substring(51, image.length())));
		} catch (IOException e) {
			e.printStackTrace();
		}
